...
(function () {
  ...
  const INTERNAL_URL_PATTERN =
    /^(chrome|vivaldi|devtools|chrome-extension):\/\//;
  ...
  const colorUtils = {
    ...
    shouldSkipPixel(r, g, b, a) {
      return (
        a < CONFIG.ALPHA_THRESHOLD ||
        (r > CONFIG.LIGHT_COLOR_THRESHOLD &&
          g > CONFIG.LIGHT_COLOR_THRESHOLD &&
          b > CONFIG.LIGHT_COLOR_THRESHOLD) ||
        (r < CONFIG.DARK_COLOR_THRESHOLD &&
          g < CONFIG.DARK_COLOR_THRESHOLD &&
          b < CONFIG.DARK_COLOR_THRESHOLD)
      );
    },
  ...
  const cssVariables = {
    setColor(colorData) {
      ...
      const textColor = colorUtils.shouldUseLightText(r, g, b)
        ? "#ffffff"
        : "#000000";
