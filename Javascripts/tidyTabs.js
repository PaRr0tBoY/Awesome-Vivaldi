(function () {
  ...
  const getUrlFragments = (url) => {
    try {
      if (
        typeof vivaldi !== "undefined" &&
        vivaldi.utilities?.getUrlFragments
      ) {
      ...
      chrome.tabs.update(
        ...
        function () {
          if (chrome.runtime.lastError) {
            ...
            console.log(
              `Added tab ${tabId} to stack ${stackId} (${stackName})`,
            );
          }
          resolve();
        },
  ...
  const buildAIPrompt = (tabs, existingStacks, languageName) => {
    ...
    const existingInfo =
      Array.isArray(existingStacks) && existingStacks.length > 0
        ? existingStacks
            .map(
              (s, i) =>
                `${i}. Stack title: ${s.name || "Unnamed stack"} (ID: ${s.id})`,
            )
            .join("\n")
        : "None";
...
`;
  ...
  const parseAIResponse = (content) => {
    ...
    try {
      ...
      showNotification(
        "AI returned invalid data format, cannot parse JSON. Check console for details.",
      );
      return null;
    }
  };
  ...
  const validateAIGroups = (result) => {
    if (!result.groups || !Array.isArray(result.groups)) {
      ...
      showNotification(
        "AI returned incorrect data format: missing groups array",
      );
      return false;
    }
  ...
  const handleOrphanTabs = (
    ...
    console.log(
      `Found ${orphanTabs.length} orphan tabs:`,
      orphanTabs.map((t) => t.title),
    );
    ...
    if (othersGroup) {
      ...
      console.log(
        'Adding orphan tabs to existing "Others" group from AI result',
      );
      othersGroup.tabs.push(...orphanTabs);
    } else {
      // Case B: Check original existing stacks for "Others"
      const existingOthersStack = existingStacks.find((s) =>
        OTHERS_NAMES.includes(s.name),
      );

      if (existingOthersStack) {
        console.log(
          'Adding orphan tabs to EXISTING "Others" stack from original list',
        );
        groupedTabs.push({
        ...
        const othersName = getOthersName();
        console.log(
          `Creating new "Others" group with ${orphanTabs.length} tabs`,
        );
        groupedTabs.push({
          name: othersName,
          tabs: orphanTabs,
        ...
        console.log(
          'Only 1 orphan tab found and no "Others" stack, not creating group',
        );
      }
    }
  };
  ...
  const getAIGrouping = async (tabs, existingStacks = []) => {
    ...
    if (tabs.length > CONFIG.maxTabsForAI) {
      console.warn(
        `Too many tabs (${tabs.length}), limiting to ${CONFIG.maxTabsForAI}`,
      );
    ...
    try {
      ...
      if (groupedTabs.length === 0) {
        console.warn(
          "No valid groups created (all groups have less than 2 tabs)",
        );
        showNotification(
          "AI grouping failed: all groups have less than 2 tabs",
        );
        return null;
      }
  ...
  const createTabStacks = async (groups) => {
    for (const group of groups) {
      ...
      console.log(
        `${group.isExisting ? "Adding to existing" : "Creating"} stack "${stackName}" with ${group.tabs.length} tabs`,
      );
  ...
  const detectExistingStacks = async (nextElement) => {
    ...
    while (nextElement) {
      ...
      if (isStack) {
        console.log(
          "Found existing tab stack DOM:",
          nextElement.outerHTML.slice(0, 200),
        );

        const stackWrapper = nextElement.querySelector(SELECTORS.TAB_WRAPPER);
        const stackTabId = stackWrapper
        ...
        if (stackTabId) {
          ...
          const stackTab = allTabs.find((t) => {
            try {
              ...
              return (
                data &&
                data.group &&
                t.vivExtData.includes(stackTabId.slice(0, 8))
              );
            } catch {
              return false;
            }
          ...
          if (stackTab) {
            ...
            console.log(
              `Detected existing stack: ${viv.fixedGroupTitle || stackTab.title} (ID: ${viv.group})`,
            );
          } else {
            console.warn(
              "No matching chrome tab found for DOM id:",
              stackTabId,
            );
          }
        }
  ...
  const tidyTabsBelow = async (separator) => {
    ...
    if (tabsInfo.length < 2 && existingStacks.length === 0) {
      console.log(
        "Not enough tabs to group (need at least 2) and no existing stacks",
      );
      return;
    }
    ...
    const observer = new MutationObserver(function (mutations) {
      ...
      if (hasTabChange || hasWorkspaceSwitch) {
        const delay = hasWorkspaceSwitch
          ? CONFIG.delays.workspaceSwitch
          : CONFIG.delays.mutation;
  ...
  const init = () => {
    ...
    console.log(
      "AI grouping:",
      CONFIG.enableAIGrouping ? "enabled" : "disabled",
    );
