...
(async () => {
  ...
  function highlightJson(element, text) {
    ...
    const jsonNumbers = [...text.matchAll(/-?\d+\.?\d*((E|e)[\+]\d+)?/gi)].map(
      (match) => {
      ...
      gnoh.object.merge(
        {
          class: "editor",
          contentEditable:
            attribute.contentEditable === false ? false : "plaintext-only",
          events: {
            input() {
  ...
  async function showDialogImport(commandChainText) {
    ...
    const buttonInputElement = Object.assign(
      {},
  ...
  async function showDialogExport(key) {
    ...
    const buttonCopyElement = Object.assign(
      {},
      gnoh.constant.dialogButtons.submit,
      {
        label: langs.copy,
