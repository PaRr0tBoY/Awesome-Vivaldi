...
var PIP = {
  ...
  videoOver: function (event) {
    ...
    if (videoElement) {
      ...
      if (foundVideo) {
        ...
        if (
          this.containerElm_ &&
          !this.containerElm_.classList.contains("transparent") &&
          !this.containerElm_.matches(":hover")
        ) {
          this.createTimer();
        }
      }
  ...
  videoOut: function (event) {
    ...
    if (
      this.containerElm_ &&
      !this.containerElm_.contains(event.relatedTarget) &&
      !this.pipButton_.contains(event.relatedTarget)
    ) {
      this.createTimer();
    }
  },
  ...
  doVideoOver: function (x, y, videoElement) {
    ...
    if (document.fullscreenElement || !this.containerElm_ || !this.pipButton_) {
      if (this.containerElm_)
        this.containerElm_.classList.add("transparent", "fullscreen"); // Hide in fullscreen
      return;
    }
    ...
    if (video && video.readyState > 1) {
      ...
      const left =
        rect.left + rect.width / 2 - buttonActualWidth / 2 + window.scrollX;
  ...
  onPipExit: function (eventOrVideo) {
    ...
    if (
      videoExiting &&
      typeof videoExiting.removeEventListener === "function"
    ) {
  ...
  pipClicked: function (event) {
    ...
    if (videoTarget) {
      ...
      if (event) {
        // Prevent default action if it was a mouse click
    ...
    try {
      ...
      navigator.mediaSession.setActionHandler("play", () => {
        if (video.paused)
          video
            .play()
            .catch((e) => console.error("PiP ERROR: Play action failed:", e));
      });
          ...
          if (mutation.type === "childList") {
            mutation.addedNodes.forEach((node) => {
              if (node.nodeType === Node.ELEMENT_NODE) {
                if (
                  node.tagName === "VIDEO" ||
                  (node.matches && node.matches("video")) ||
