...
(() => {
  ...
  const code =
    "data:text/html," + encodeURIComponent("<title>" + name + "</title>");
  ...
  const icons = {
    ...
    closePanel:
      '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"><path d="m12.5 5-1.4-1.4-3.1 3-3.1-3L3.5 5l3.1 3.1-3 2.9 1.5 1.4L8 9.5l2.9 2.9 1.5-1.4-3-2.9"/></svg>',
    pictureInPicture: {
    ...
    volume: {
      ...
      medium:
        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="currentColor" d="M5,9V15H9L14,20V4L9,9M18.5,12C18.5,10.23 17.5,8.71 16,7.97V16C17.5,15.29 18.5,13.76 18.5,12Z"/></svg>',
    ...
    sidebar: {
      ...
      right:
        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="currentColor" d="M20 4H4A2 2 0 0 0 2 6V18A2 2 0 0 0 4 20H20A2 2 0 0 0 22 18V6A2 2 0 0 0 20 4M15 18H4V6H15Z"/></svg>',
    },
  };
  ...
  function injectMain(messageType, nameAttribute) {
    ...
    const addEventListenerVideoOriginal =
    ...
    const addEventListenerAudioOriginal =
    ...
    function isPlaying(video) {
      return (
        ...
        video.getAttribute(nameAttribute)
      );
    }
    ...
    function getDataControl(video) {
      return {
        ...
        paused: video.paused,
        audio:
          !video.webkitVideoDecodedByteCount || video.disablePictureInPicture,
    ...
    function timeupdateVideo(event) {
      ...
      if (enable) {
        if (
          event.target.paused &&
          !event.target.webkitAudioDecodedByteCount &&
          !event.target.webkitVideoDecodedByteCount
        ) {
          endedVideo(event);
    ...
    function addEventListeners(video) {
      ...
      addEventListenerAudioOriginal.apply(video, [
        "timeupdate",
        timeupdateVideo,
      ]);
      addEventListenerAudioOriginal.apply(video, [
      ...
      addEventListenerAudioOriginal.apply(video, [
        ...
        enterpictureinpictureVideo,
      ]);
      addEventListenerAudioOriginal.apply(video, [
        "leavepictureinpicture",
        leavepictureinpictureVideo,
      ]);
    }
    ...
    if (!colorLoaded[image.src]) {
      ...
      if (filterEntryColors.length > 0) {
        ...
        filterEntryColors = entryColors.filter((entryColor) => {
          ...
          return (
            gnoh.color.deltaE({ r: 0, g: 0, b: 0 }, { r: +r, g: +g, b: +b }) >
              2 &&
            gnoh.color.deltaE(
              { r: 255, g: 255, b: 255 },
              { r: +r, g: +g, b: +b }
      ...
      const rgbProgressBar = gnoh.color.shadeColor(
        rgbMax.r,
        rgbMax.g,
        rgbMax.b,
        isLightBg ? 0.4 : -0.4
  ...
  function createItem(tab, info) {
    const itemInfo = {
      ...
      setImage(src) {
        if (
          itemInfo.image !== undefined &&
          (src == null || src === itemInfo.image)
        ) {
          return;
        }
        itemInfo.image = src;
      ...
      setPaused(paused) {
        if (paused != null && itemInfo.paused !== paused) {
          ...
          itemInfo.buttonControl.innerHTML = itemInfo.paused
            ? icons.play
            : icons.pause;
        }
      },
      setPictureInPicture(pictureInPicture) {
        if (
          ...
          itemInfo.pictureInPicture = pictureInPicture;
          itemInfo.buttonPictureInPicture.innerHTML = itemInfo.pictureInPicture
            ? icons.pictureInPicture.on
            : icons.pictureInPicture.off;
      ...
      setActive(active) {
        if (active != null && itemInfo.active !== active) {
          ...
          if (itemInfo.webPanelId) {
            ...
            itemInfo.buttonTab.innerHTML = itemInfo.active
              ? icons.tab.on
              : icons.tab.off;
            if (active) {
      ...
      setProgress(duration, currentTime) {
        if (
          (duration != null && itemInfo.duration !== duration) ||
          (currentTime != null && itemInfo.currentTime !== currentTime)
        ) {
          ...
          itemInfo.currentTimeDuration.textContent =
    ...
    itemInfo.buttonVolume = gnoh.createElement(
      "button",
      {
        type: "button",
        tabindex: -1,
    ...
    itemInfo.actionItem = gnoh.createElement(
      "div",
      {
        class: "action",
      },
      null,
      [
        itemInfo.buttonControl,
        itemInfo.buttonPictureInPicture,
        itemInfo.buttonTab,
        itemInfo.volumeControl,
  ...
  async function createPanelCustom(panel, webviewButton) {
    if (!chrome.extension.inIncognitoContext) {
      ...
      const buttonClose = gnoh.createElement("button", {
        ...
        style: {
          display:
            (showCloseButton && asOverlayEnabled && autoClose) ||
            !showCloseButton
              ? "none"
              : "flex",
        },
      ...
      const toolbar = gnoh.createElement(
        "div",
        {
          class: "toolbar",
        },
  ...
  const style = !chrome.extension.inIncognitoContext
    ? [
        ...
        'button[name="' +
          webPanelId +
          '"]:before { width: 16px; height: 16px; content: ""; background-color: var(--colorFg); -webkit-mask-box-image: url(' +
          JSON.stringify(icons.dataURLs.playlistMusic) +
          "); }",
        '.color-behind-tabs-off .toolbar-mainbar button[name="' +
          webPanelId +
          '"]:before { background-color: var(--colorAccentFg); }',
        '.button-toolbar:active button[name="' +
          webPanelId +
          '"]:before { transform: scale(0.9); }',
      ]
    : [
        '.button-toolbar:has(button[name="' +
          webPanelId +
          '"]) { display:none !important; }',
        '.draggable-button:has(button[name="' +
          webPanelId +
          '"]) { display:none !important; }',
      ];
  ...
  function updateIconAndTitle() {
    ...
    const webPanelStack =
      gnoh
        .getReactProps(".panel-group .webpanel-stack")
        ?.children?.filter((webPanel) => webPanel) ?? [];
    const webPanelIndex =
      webPanelStack.findIndex((webPanel) => webPanel.key === webPanelId) + 1;
  ...
  if (!chrome.extension.inIncognitoContext) {
    ...
    chrome.runtime.onMessage.addListener(async (info, sender, sendResponse) => {
      if (
        ...
        (!sender.tab || (sender.tab && !sender.tab.incognito))
      ) {
        switch (info.action) {
          case "open-webpanel":
            if (info.windowId === vivaldiWindowId) {
              simulateWebviewButtonClick({
                webPanelId: info.webPanelId,
                openOnly: true,
              });
            }
            break;
    ...
    function injectToggleLucidModeVideo(enable) {
      ...
      if (style && !enable) {
        ...
        style.innerHTML =
          'video { filter: url(\'data:image/svg+xml, <svg xmlns="http://www.w3.org/2000/svg"> <filter id="sharpen"> <feConvolveMatrix order="3" preserveAlpha="true" kernelMatrix="1 -1 1 -1 -1 -1 1 -1 1"/> </filter> </svg>#sharpen\'); }';
