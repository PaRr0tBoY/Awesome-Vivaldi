...
(() => {
  ...
  class DialogMod {
     ...
     */
    dialogTab(linkUrl, fromPanel = undefined, rect = null) {
    ...
    /**
     * 完全重写的 showDialog 方法 - 使用 Web Animations API 实现精确的 Arc Peek 效果
     */
    showDialog(linkUrl, fromPanel, rect) {
      ...
      const webpageContainer =
        document.querySelector("#browser .inner .webpageview") ||
        document.querySelector(".webpageview") ||
        document.querySelector("#browser .inner") ||
        document.querySelector("#main");

      if (!webpageContainer) {
        console.error("Cannot find suitable container for dialog");
        return;
      }

      // 获取容器的位置信息，用于坐标转换
      const containerRect = webpageContainer.getBoundingClientRect();
      console.log("Container rect:", containerRect);

      if (rect) {
        console.log("Original rect from page:", rect);

        // rect 是相对于页面 viewport 的坐标，需要转换为相对于容器的坐标
        // 由于我们的 dialog-container 是 position: absolute 相对于 webpageContainer
        // 所以起始位置应该相对于容器计算
        startRect = {
          left: rect.left - containerRect.left, // 转换为相对于容器的坐标
          top: rect.top - containerRect.top,
          width: rect.width,
          height: rect.height,
        };

        console.log("Container-relative start rect:", startRect);
      } else {
        // 如果没有 rect，从容器中心开始
        startRect = {
          left: containerRect.width / 2 - 50,
          top: containerRect.height / 2 - 25,
          width: 100,
          height: 50,
        };
      }
      ...
      endRect = {
        width: Math.min(containerWidth * 0.5, 1200),
        height: containerHeight - margin * 2,
        left: 0,
        top: margin,
      };
      endRect.left = (containerWidth - endRect.width) / 2;

      console.log("Container-relative end rect:", endRect);
      ...
      dialogTab.addEventListener("click", (e) => e.stopPropagation());

      // ！重要：初始设置位置时不要有任何 CSS transition
      Object.assign(dialogTab.style, {
        position: "absolute",
        ...
        opacity: "0",
        transform: "scale(0.9)",
        transition: "none", // 确保初始位置设置时没有动画
      });

      // Options container
      optionsContainer.setAttribute("class", "options-container");
      optionsContainer.innerHTML = this.iconUtils.ellipsis;
      optionsContainer.style.opacity = "0"; // 初始隐藏
      ...
      // 插入到网页容器中，避免覆盖工具栏
      webpageContainer.appendChild(dialogContainer);

      this.webviews.get(webviewId).divContainer = dialogContainer;

      // ========================
      // 使用 Web Animations API 实现精确动画
      // ========================

      // 强制 reflow 确保初始状态被渲染
      dialogTab.offsetHeight;
      ...
      const expandOptions = {
        duration: 320,
        easing: "cubic-bezier(0.16, 1, 0.3, 1)", // Arc-like easing
        fill: "forwards",
      };

      // 启动展开动画
      const expandAnimation = dialogTab.animate(expandKeyframes, expandOptions);

      // 同时启动背景动画
      requestAnimationFrame(() => {
        dialogContainer.classList.add("show-overlay");
      ...
      const startClose = () => {
        ...
        console.log("Starting close animation"); // 调试日志

        // 立即移除背景效果
        document.body.classList.remove("dialog-open");
        dialogContainer.classList.remove("show-overlay");

        // 隐藏选项
        optionsContainer.style.opacity = "0";

        // 获取当前位置（动画可能正在进行中）
        const currentRect = dialogTab.getBoundingClientRect();
        ...
        const collapseOptions = {
          duration: 260,
          easing: "cubic-bezier(0.4, 0, 0.6, 1)", // 收缩稍快
          fill: "forwards",
        };
        ...
        // 启动收缩动画
        const collapseAnimation = dialogTab.animate(
          collapseKeyframes,
          collapseOptions,
        );
        ...
        // 等待收缩动画完成
        collapseAnimation.addEventListener("finish", cleanup);
      ...
      // 存储清理函数
      this.webviews.get(webviewId).startClose = startClose;
    }
  ...
  class WebsiteLinkInteractionHandler {
        ...
        this.debounce((event) => {
          ...
          requestAnimationFrame(() => {
            Object.assign(this.icon.style, {
              display: "block",
              left: `${linkRect.right + 5}px`,
              top: `${linkRect.top + window.scrollY}px`,
            });
          });

          this.icon.dataset.targetUrl = link.href;
          // 保存rect信息供icon点击/hover时使用
          this.icon.dataset.targetRect = JSON.stringify(rect);
      ...
      if (this.config.linkIconInteractionOnHover) {
        icon.addEventListener("mouseenter", () => {
          this.timers.showDialog = setTimeout(() => {
            const rect = this.icon.dataset.targetRect
              ? JSON.parse(this.icon.dataset.targetRect)
              : null;
            this.#sendDialogMessage(this.icon.dataset.targetUrl, rect);
    ...
    #sendDialogMessage(url, rect) {
      // 发送相对于页面viewport的rect（宿主将转换为宿主坐标）
      chrome.runtime.sendMessage({
        url,
        fromPanel: this.fromPanel,
        rect: rect || null,
    ...
    #callDialog(event) {
      ...
      if (link) {
        ...
        // 获取链接相对于页面viewport的位置
        const linkRect = link.getBoundingClientRect();

        // 重要：构建准确的位置信息
        const rect = {
          left: linkRect.left, // 已经是相对于viewport的位置
          top: linkRect.top, // 已经是相对于viewport的位置
          width: linkRect.width,
          height: linkRect.height,
        };

        console.log("Link clicked, rect:", rect); // 调试日志
        console.log("Link element:", link); // 调试日志
