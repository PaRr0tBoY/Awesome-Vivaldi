...
(() => {
  ...
  class DialogMod {
    ...
    showDialog(linkUrl, fromPanel, rect) {
      console.log('showDialog called with rect:', rect); // 调试日志
      ...
      if (rect) {
        console.log('Processing rect:', rect); // 调试日志
        ...
        if (hostWebviewEl) {
          ...
          console.log('Host rect:', hostRect); // 调试日志
          console.log('Calculated start position:', { left: startLeft, top: startTop }); // 调试日志
          ...
          console.log('No host webview found, using original rect'); // 调试日志
        ...
        console.log('No rect provided to showDialog'); // 调试日志
      }
      ...
      setTimeout(() => {
        ...
        console.log('Added animating-in class to dialogTab'); // 调试日志
      }, 10);
      ...
      const startClose = () => {
        ...
        if (saved) {
          ...
          // 移除 animating-in，添加 animating-out 类
          dialogTab.classList.remove("animating-in");
          dialogTab.classList.add("animating-out");

          // 设置回到起始宿主坐标，但 transition 会处理平滑过渡
          setTimeout(() => {
          ...
          dialogTab.classList.remove("animating-in");
          dialogTab.classList.add("animating-out");
          dialogTab.style.left = "50%";
          dialogTab.style.top = "50%";
          dialogTab.style.width = "0";
          dialogTab.style.height = "0";
        }
  ...
  class WebsiteInjectionUtils {
    constructor(getWebviewConfig, openDialog, iconConfig) {
      ...
      chrome.runtime.onMessage.addListener((message) => {
        console.log('Message received:', message); // 调试日志
        if (message.url) {
          console.log('Calling openDialog with rect:', message.rect); // 调试日志
  ...
  class WebsiteLinkInteractionHandler {
    ...
    #sendDialogMessage(url, rect) {
      console.log('Sending dialog message with rect:', rect); // 调试日志
