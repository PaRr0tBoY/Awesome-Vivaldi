 ...
 *
 * 新增功能：长按左键打开弹窗
 * - 长按左键 400ms 可以打开链接在弹窗中
 * - 提供200ms延迟显示的环形进度条，避免误触发
 * - 进度环颜色从白色渐变到绿色
 * - 支持自定义进度环大小、线宽和延迟时间
 * - 配置选项：
 *   - leftClickHoldTime: 长按总时间（400ms）
 *   - leftClickHoldDelay: 显示进度环的延迟时间（200ms）
 *   - progressRingRadius: 进度环半径（25px）
 *   - progressRingWidth: 进度环线宽（3px）
 */
(() => {
  const ICON_CONFIG = {
      ...
      leftClickHoldTime: 400, // 长按左键的时间（毫秒）来打开dialogTab
      leftClickHoldDelay: 200, // 长按左键后延迟显示进度环的时间（毫秒）
      progressRingRadius: 20, // 进度环的半径
      progressRingWidth: 3, // 进度环的线宽
    },
    CONTEXT_MENU_CONFIG = {
      menuPrefix: "[Peek]",
      linkMenuTitle: "显示弹窗",
        ...
        const clearWebviews = (closedTabId) => {
          if (tabId === closedTabId) {
            this.webviews.forEach(
              (view, key) =>
                view.tabCloseListener === clearWebviews && startClose(),
            );
        ...
        if (event.altKey && [0, 1].includes(event.button)) {
          ...
          this.visibilityDelayTimer = setTimeout(() => {
            ...
            this.progressInterval = setInterval(() => {
              ...
              // 根据进度改变颜色（从白色到绿色）
              const hue = 120 * progress; // 从0度（红色）到120度（绿色）
              const saturation = 100;
              const lightness = 50 + 50 * (1 - progress); // 从50%亮度变化
              this.progressCircle.setAttribute(
                "stroke",
